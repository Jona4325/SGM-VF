...
def index(request):
    ...
    """
    return render(request, 'jap/index.html')
...
def coordinator_list(request):
    ...
    print(coordinators)
    return render(request, 'jap/coordinator_list.html', {'coordinators': coordinators})
...
def coordinator_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('jap:coordinator_list')
    else:
        form = CoordinatorForm()
    return render(request, 'jap/coordinator_form.html', {'form': form, 'action': 'Crear'})
...
def coordinator_update(request, pk):
    ...
    return render(request, 'jap/coordinator_form.html', {'form': form, 'action': 'Actualizar'})
...
def coordinator_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('jap:coordinator_list')
    return render(request, 'jap/coordinator_confirm_delete.html', {'coordinator': coordinator_obj})

# vista para crear groups
@login_required
def group_list(request):
    groups = group.objects.all()
    return render(request, 'jap/group_list.html', {'groups': groups})
...
def group_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('jap:group_list')
    else:
        form = GroupForm()
    return render(request, 'jap/group_form.html', {'form': form})
...
def group_delete(request, pk):
    ...
    if request.method == 'POST':
        group_obj.delete()
        return redirect('jap:group_list')
    return render(request, 'jap/group_confirm_delete.html', {'group': group_obj})

class ServerListView(LoginRequiredMixin,ListView):
    model = server
    template_name = 'jap/server_list.html'  # Especifica tu template
    context_object_name = 'servers'  # Nombre de la variable en el template

class ServerDetailView(LoginRequiredMixin,DetailView):
    model = server
    template_name = 'jap/server_detail.html'
    context_object_name = 'server'

class ServerCreateView(LoginRequiredMixin,CreateView):
    model = server
    form_class = ServerForm
    template_name = 'jap/server_form.html'
    success_url = reverse_lazy('jap:server_list') # Redirige a la lista después de crear
...
class ServerUpdateView(LoginRequiredMixin,UpdateView):
    ...
    form_class = ServerForm
    template_name = 'jap/server_form.html'
    success_url = reverse_lazy('jap:server_list') # Redirige a la lista después de actualizar
...
class ServerDeleteView(LoginRequiredMixin,DeleteView):
    model = server
    template_name = 'jap/server_confirm_delete.html'
    context_object_name = 'server'
    success_url = reverse_lazy('jap:server_list') # Redirige a la lista después de borrar
...
def child_list(request):
    ...
    return render(request, 'jap/child_list.html', {'children': children_list})
@login_required
def child_create(request): # <-- Renombrada
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('jap:child_list') # <-- Cambiado
    else:
        form = ChildForm() # <-- Cambiado
    # Asegúrate que el template existe o renómbralo
    return render(request, 'jap/child_form.html', {'form': form, 'action': 'Registrar'}) # <-- Cambiado template
...
def child_update(request, pk): # <-- Renombrada
    ...
    # Asegúrate que el template existe o renómbralo
    return render(request, 'jap/child_form.html', {'form': form, 'action': 'Actualizar', 'action_title': action_title}) # <-- Cambiado template
...
def child_delete(request, pk): # <-- Renombrada
    ...
    if request.method == 'POST':
        ...
        return redirect('jap:child_list') # <-- Cambiado
    # Asegúrate que el template existe o renómbralo
    return render(request, 'jap/child_confirm_delete.html', {'child': child_obj}) # <-- Cambiado template y contexto
...
def assistance_list(request):
    ...
    return render(request, 'jap/assistance_list.html', {'assistances': assistances_list})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect('jap:assistance_list')
                ...
                return render(request, 'jap/assistance_batch_form.html', {
                    'form': form,
...
def assistance_update(request, pk):
    ...
    return render(request, 'jap/assistance_form.html', {
        'form': form,
        'action': 'Actualizar',
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('jap:assistance_list')
             ...
             return render(request, 'jap/assistance_confirm_delete.html', {
                 'assistance': assistance_obj,
             })

    return render(request, 'jap/assistance_confirm_delete.html', {'assistance': assistance_obj})

# --- Vistas para el CRUD de GroupCoordinator (jap) ---
@login_required
def groupcoordinator_list(request):
    ...
    return render(request, 'jap/groupcoordinator_list.html', {'assignments': assignments})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect('jap:groupcoordinator_list')
            except IntegrityError:
                messages.error(request, 'Error: Esta asignación de grupo y coordinador ya existe.')
    else:
        form = GroupCoordinatorForm()
    return render(request, 'jap/groupcoordinator_form.html', {'form': form, 'action': 'Asignar'})
...
def groupcoordinator_update(request, pk):
    ...
    return render(request, 'jap/groupcoordinator_form.html', {
        'form': form, 
        'action': 'Actualizar', 
        'action_title': action_title
    })
...
def groupcoordinator_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('jap:groupcoordinator_list')
    return render(request, 'jap/groupcoordinator_confirm_delete.html', {'assignment': assignment})
