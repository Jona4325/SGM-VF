...
def index(request):
    ...
    """
    return render(request, 'jpro/index.html')
...
def coordinator_list(request):
    ...
    print(coordinators)
    return render(request, 'jpro/coordinator_list.html', {'coordinators': coordinators})
...
def coordinator_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('jpro:coordinator_list')
    else:
        form = CoordinatorForm()
    return render(request, 'jpro/coordinator_form.html', {'form': form, 'action': 'Crear'})
...
def coordinator_update(request, pk):
    ...
    return render(request, 'jpro/coordinator_form.html', {'form': form, 'action': 'Actualizar'})
...
def coordinator_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('jpro:coordinator_list')
    return render(request, 'jpro/coordinator_confirm_delete.html', {'coordinator': coordinator_obj})

# vista para crear groups
@login_required
def group_list(request):
    groups = group.objects.all()
    return render(request, 'jpro/group_list.html', {'groups': groups})
...
def group_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('jpro:group_list')
    else:
        form = GroupForm()
    return render(request, 'jpro/group_form.html', {'form': form})
...
def group_delete(request, pk):
    ...
    if request.method == 'POST':
        group_obj.delete()
        return redirect('jpro:group_list')
    return render(request, 'jpro/group_confirm_delete.html', {'group': group_obj})

class ServerListView(LoginRequiredMixin,ListView):
    model = server
    template_name = 'jpro/server_list.html'  # Especifica tu template
    context_object_name = 'servers'  # Nombre de la variable en el template

class ServerDetailView(LoginRequiredMixin,DetailView):
    model = server
    template_name = 'jpro/server_detail.html'
    context_object_name = 'server'

class ServerCreateView(LoginRequiredMixin,CreateView):
    model = server
    form_class = ServerForm
    template_name = 'jpro/server_form.html'
    success_url = reverse_lazy('jpro:server_list') # Redirige a la lista después de crear
...
class ServerUpdateView(LoginRequiredMixin,UpdateView):
    ...
    form_class = ServerForm
    template_name = 'jpro/server_form.html'
    success_url = reverse_lazy('jpro:server_list') # Redirige a la lista después de actualizar
...
class ServerDeleteView(LoginRequiredMixin,DeleteView):
    model = server
    template_name = 'jpro/server_confirm_delete.html'
    context_object_name = 'server'
    success_url = reverse_lazy('jpro:server_list') # Redirige a la lista después de borrar
...
def child_list(request):
    ...
    return render(request, 'jpro/child_list.html', {'children': children_list})
@login_required
def child_create(request): # <-- Renombrada
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('jpro:child_list') # <-- Cambiado
    else:
        form = ChildForm() # <-- Cambiado
    # Asegúrate que el template existe o renómbralo
    return render(request, 'jpro/child_form.html', {'form': form, 'action': 'Registrar'}) # <-- Cambiado template
...
def child_update(request, pk): # <-- Renombrada
    ...
    # Asegúrate que el template existe o renómbralo
    return render(request, 'jpro/child_form.html', {'form': form, 'action': 'Actualizar', 'action_title': action_title}) # <-- Cambiado template
...
def child_delete(request, pk): # <-- Renombrada
    ...
    if request.method == 'POST':
        ...
        return redirect('jpro:child_list') # <-- Cambiado
    # Asegúrate que el template existe o renómbralo
    return render(request, 'jpro/child_confirm_delete.html', {'child': child_obj}) # <-- Cambiado template y contexto
...
def assistance_list(request):
    ...
    return render(request, 'jpro/assistance_list.html', {'assistances': assistances_list})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect('jpro:assistance_list')
                ...
                return render(request, 'jpro/assistance_batch_form.html', {
                    'form': form,
...
def assistance_update(request, pk):
    ...
    return render(request, 'jpro/assistance_form.html', {
        'form': form,
        'action': 'Actualizar',
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('jpro:assistance_list')
             ...
             return render(request, 'jpro/assistance_confirm_delete.html', {
                 'assistance': assistance_obj,
             })

    return render(request, 'jpro/assistance_confirm_delete.html', {'assistance': assistance_obj})

# --- Vistas para el CRUD de GroupCoordinator (jpro) ---
@login_required
def groupcoordinator_list(request):
    ...
    return render(request, 'jpro/groupcoordinator_list.html', {'assignments': assignments})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect('jpro:groupcoordinator_list')
            except IntegrityError:
                messages.error(request, 'Error: Esta asignación de grupo y coordinador ya existe.')
    else:
        form = GroupCoordinatorForm()
    return render(request, 'jpro/groupcoordinator_form.html', {'form': form, 'action': 'Asignar'})
...
def groupcoordinator_update(request, pk):
    ...
    return render(request, 'jpro/groupcoordinator_form.html', {
        'form': form, 
        'action': 'Actualizar', 
        'action_title': action_title
    })
...
def groupcoordinator_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('jpro:groupcoordinator_list')
    return render(request, 'jpro/groupcoordinator_confirm_delete.html', {'assignment': assignment})
