...
def index(request):
    ...
    """
    return render(request, 'pequediks/index.html')
...
def coordinator_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('pequediks:coordinator_list')
    else:
        form = CoordinatorForm()
    return render(request, 'pequediks/coordinator_form.html', {'form': form, 'action': 'Crear'})
...
def coordinator_update(request, pk):
    ...
    return render(request, 'pequediks/coordinator_form.html', {'form': form, 'action': 'Actualizar'})
...
def coordinator_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('pequediks:coordinator_list')
    return render(request, 'pequediks/coordinator_confirm_delete.html', {'coordinator': coordinator_obj})
...
def group_create(request):
    if request.method == 'POST':
        ...
        form = GroupForm()
    return render(request, 'pequediks/group_form.html', {'form': form})
...
class ServerDetailView(LoginRequiredMixin,DetailView):
    model = server
    template_name = 'pequediks/server_detail.html'
    context_object_name = 'server'

class ServerCreateView(LoginRequiredMixin,CreateView):
    model = server
    form_class = ServerForm
    template_name = 'pequediks/server_form.html'
    success_url = reverse_lazy('pequediks:server_list') # Redirige a la lista después de crear
...
class ServerUpdateView(LoginRequiredMixin,UpdateView):
    ...
    form_class = ServerForm
    template_name = 'pequediks/server_form.html'
    success_url = reverse_lazy('pequediks:server_list') # Redirige a la lista después de actualizar
...
class ServerDeleteView(LoginRequiredMixin,DeleteView):
    model = server
    template_name = 'pequediks/server_confirm_delete.html'
    context_object_name = 'server'
    success_url = reverse_lazy('pequediks:server_list') # Redirige a la lista después de borrar
...
def child_create(request): # <-- Renombrada
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('pequediks:child_list') # <-- Cambiado
    else:
        form = ChildForm() # <-- Cambiado
    # Asegúrate que el template existe o renómbralo
    return render(request, 'pequediks/child_form.html', {'form': form, 'action': 'Registrar'}) # <-- Cambiado template
...
def child_update(request, pk): # <-- Renombrada
    ...
    # Asegúrate que el template existe o renómbralo
    return render(request, 'pequediks/child_form.html', {'form': form, 'action': 'Actualizar', 'action_title': action_title}) # <-- Cambiado template
...
def child_delete(request, pk): # <-- Renombrada
    ...
    if request.method == 'POST':
        ...
        return redirect('pequediks:child_list') # <-- Cambiado
    # Asegúrate que el template existe o renómbralo
    return render(request, 'pequediks/child_confirm_delete.html', {'child': child_obj}) # <-- Cambiado template y contexto
...
def assistance_update(request, pk):
    ...
    return render(request, 'pequediks/assistance_form.html', {
        'form': form,
        'action': 'Actualizar',
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('pequediks:assistance_list')
             ...
             return render(request, 'pequediks/assistance_confirm_delete.html', {
                 'assistance': assistance_obj,
             })

    return render(request, 'pequediks/assistance_confirm_delete.html', {'assistance': assistance_obj})

# --- Vistas para el CRUD de GroupCoordinator (pequediks) ---
@login_required
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect('pequediks:groupcoordinator_list')
            except IntegrityError:
                messages.error(request, 'Error: Esta asignación de grupo y coordinador ya existe.')
    else:
        form = GroupCoordinatorForm()
    return render(request, 'pequediks/groupcoordinator_form.html', {'form': form, 'action': 'Asignar'})
...
def groupcoordinator_update(request, pk):
    ...
    return render(request, 'pequediks/groupcoordinator_form.html', {
        'form': form, 
        'action': 'Actualizar', 
        'action_title': action_title
    })
...
def groupcoordinator_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('pequediks:groupcoordinator_list')
    return render(request, 'pequediks/groupcoordinator_confirm_delete.html', {'assignment': assignment})
