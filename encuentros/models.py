...
class Meeting(TenantAwareModel): # Renombrado de Encounter a Meeting
    """
...
class Participant(TenantAwareModel):
    STATUS_CHOICES = [
        ('SOLTERO', _('Single')),
        ('CASADO', _('Married')),
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name='encuentros_participant_tenant_email_uniq',
            ),
        ]

class FamilyParticipantInfo(TenantAwareModel):
...
class ChurchDataInfo(TenantAwareModel):
    participant = models.ForeignKey(
        Participant,
        on_delete=models.PROTECT,
...
class MeetingParticipant(TenantAwareModel):
    ...
    class Meta:
        ...
        unique_together = [['tenant', 'meeting', 'participant', 'event_date']] # Ensure one record per participant per meeting on a given date

    def __str__(self):
        return f"{self.participant} at {self.meeting} on {self.event_date}"


class FinanceMovements(TenantAwareModel):
    TRANS_CHOICES = [
        ('INCOME', _('Income')),
        ('EXPENSE', _('Expense')),
    ]
...
class Summary(TenantAwareModel):
